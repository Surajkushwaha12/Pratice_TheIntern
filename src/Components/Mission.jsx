...
function Card({ title, content }) {
  ...
  return (
    <div className="p-6 md:p-10 max-h-full md:max-h-[320px] bg-white cursor-pointer hover:scale-105 transition-all duration-200 rounded-xl border-2"
    >
      <h3 className="font-bold pt-4 text-2xl">{title}</h3>
      <div className={`py-3 leading-normal text-grayText overflow-hidden ${expanded ? 'h-auto' : 'h-[93px]'} ${expanded ? '' : 'line-clamp-3'}`}>        <p className="h-full text-lg">{content}</p>
      </div>
...
function MissionSection() {
  return (
     <section className="bg-white relative font-mullish">
      <div className="w-11/12 mx-auto mt-8 max-w-[1280px] flex flex-col md:flex-row justify-center items-center gap-y-10 gap-x-6">
        <Card
          title="Our Mission"
          content="At The Interns, our mission is clear: to nurture the growth of students and empower them to achieve their full potential. We understand the importance of education in today's rapidly evolving world, and we are dedicated to providing top-notch online courses that cater to the diverse needs of learners worldwide. We aspire to create a dynamic learning community."
        />
        <Card
          title="Our Vision"
